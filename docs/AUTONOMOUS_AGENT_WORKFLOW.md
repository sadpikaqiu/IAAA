...
核对日期：2026-09-22。本文说明当前已经实现的 `autonomous` 引擎（实验中的 D 组），并说明它与固定流程、实验调度器之间的关系。
...
2026-09-22 fix04 增补：修正 tokenizer 返回 `BatchEncoding` 时把字段数误当 token 数的问题；B/C/D 公共排名输出改为 POI ID 键与模型显式名次，约束唯一候选及同 POI 引用；评估改成最多 16 个在途会话的滚动调度。这些变化使用新的冻结协议重新验证，旧结果保留在原目录。

同日 fix05 增补：fix04 的显式名次在 NYC 四个会话中重复，耗尽修复预算。fix05 恢复有序 `ranked_pois` 数组；若唯一错误是重复 POI，则保留首次出现的合法条目，由模型只补选缺位并追加到末尾。补选从生成 Schema 中排除保留 ID，仍计入原有两次共享修复预算。token 计数修复与 16 并发滚动调度保留。不同协议的正式预测不混用。

同日 fix06 增补（用户授权）：验证和全量阶段允许已分类的终止失败，按组隔离并保留在端到端指标分母内；development 与 smoke50 仍须全部成功。生成提示、候选、引用、模型参数和预算不变。fix05 已完成结果只在逐文件哈希、生成代码兼容性、数据和配置核验通过后继承，保存原请求身份与来源；新统计结果使用独立协议和目录，不修改 fix05 的报告。
...
图中的“修复”需要重新请求模型。fix05 可记录重复 POI 并保留首次出现的合法条目，但缺位必须由模型补选，程序不会从原候选池按顺序直接填入。
...
当前 fix05 使用 `ranked_pois` 有序数组，列表顺序就是模型偏好顺序，不另行生成整数名次。数组元素的 Schema 按候选分支限定 POI ID 及其可引用证据。原始 JSON 仍拒绝重复键；数组 POI 的唯一性由运行时检查。此前 fix04 使用 ID 键和显式 `rank` 的协议仅保留历史重放支持，不再用于新预测。

对于条数、字段、候选和每条引用均合法、但 POI 重复的答案，保留每个 ID 首次出现的完整条目及相对顺序。例如 `[A, B, A, C, ...]` 去重后不足 10 个，补选请求继续提供原输入事实，只要求返回缺少的条数，并从允许输出的候选 Schema 中排除全部保留 ID。模型返回的合法补选按其顺序追加到末尾。它可以选择原候选池中任何未保留的合法候选，程序不依据真值、候选池顺序或统计得分选替代项。

补选若自身仍重复，只有在其余检查全部通过后才保留首次出现的新增项，并在剩余预算内继续补选；引用错误、越界、截断等响应不能贡献新条目。只要未形成完整合法 Top-10，就不输出成功预测。这是明确记录的修复协议，不把补选结果冒充首轮无错响应。
...
fix04 显式指定 `return_dict=False`，并兼容字典返回值中的 `input_ids`；每次请求记录估计输入 token 数。此前服务器 tokenizer 返回两个字段的 `BatchEncoding`，旧代码的 `len(...)` 得到 2，导致预算检查实际失效，不能把旧版日志理解为已经通过正确的长度检查。
...
常规修复请求会尽可能带上上一版错误 JSON，并附具体诊断。若完整错误答案放不下，则缩为 ID/引用摘要；必要时仅保留诊断，原始事实继续保留。最终排名的重复 POI 使用第 8 节的缺位补选机制，要求只返回缺位，不重写已保留条目。其他错误继续使用整份回答修复；同时有重复和引用错误的答案不会被当作“仅重复”接受部分内容。

所有决策和最终排名共同使用该运行的两次额外修复预算。不能每轮各重试两次，也不会用启发式排名兜底掩盖失败。

最终排名的首次请求与补选请求都记录在同一个 `final_ranking.json` 的 `attempts` 中，故每次补选自然计入共享修复次数。日志保存每次实际消息、动态 Schema、保留条目、排除 ID、补选数量、原始响应和 usage。中断尝试也消耗次数；恢复时从历史合法片段重建保留集合，核对已缓存请求后才合并结果，不重发已完成的补选。
...
fix06 把失败隔离到实验组。已知模型输出错误、修复预算耗尽、候选不足、上下文预算或会话超时，记录为 `model`；可识别的连接失败、HTTP 408/429/5xx、缺失响应 usage 等记录为 `infrastructure`。HTTP 400/401 等配置错误、未知异常及身份/隔离/缓存校验错误仍为致命错误，停止派发并保留现场。不得把所有 ValueError 或 RuntimeError 统一视作可忽略失败。

失败组保存 `failure.json`、空预测、原因、traceback 和真实请求用量；其他独立组继续。A 失败时，同模态 B 标记 `dependency`，C、D 及另一模态继续。若所有组均达到成功或已记录失败的终态，会话为 `completed` 或 `completed_with_failures`。恢复不会重跑终止失败组，也不会重置原有共享两次修复预算；没有额外的外层重试。

16 并发继续滚动补入。在 validation/full 中，普通已记录失败不再触发累计 4 个失败的停止条件；development/smoke50 保留该停止条件及全部成功门槛。所有阶段都检查完整预定样本与组别，未执行或被中断的组不能当作失败计零以伪装完成。

服务熔断按真实新请求的完成顺序统计：连续 5 次基础设施失败，或最近至多 20 次请求中有 8 次基础设施失败，则锁定熔断状态，停止派发新会话和新实验组，让在途组在原预算内收尾后退出为 `paused`。不停止模型服务，也不自动重试整批实验；检查服务恢复后显式重新启动脚本。模型格式错误不计入服务熔断。
...
当前服务器启动脚本使用 16 并发，即同时处理最多 16 个会话，完成一条就补入下一条，不等待整批 16 条全部结束。每个会话内部依次跑各实验组；两种模态的先后由不使用标签的稳定规则决定，A 总在需要它的 B 之前执行。命令行可单独指定并发，值与滚动调度版本均写入协议身份。

### 13.1 质量检查与推荐指标分开看

development/smoke50 仍要求全部合法 Top-10、真实模型调用、完整 usage 和零兜底。validation/full 要求完整预定会话与组别；每组必须有合法输出或可审计的已知终止失败，不能缺行、重复或伪造预测。缺失 usage 单独记录，已知 token 总量只是部分观测，不能冒充完整成本。

主要 Hit@1/5/10、NDCG@10、MRR@10 使用全部预定会话，无法返回合法推荐的终止失败计零，并报告逐组失败数、失败率和 model/infrastructure/dependency 分类。CandidateRecall 等只在有合法完整输出的会话上计算，记录独立分母；失败会话的未知候选召回不填零。`conditional_contrasts` 另外报告双方均成功的相同子集、排除数量及配对统计，只供辅助解释，不能替代端到端主要比较。

失败请求和共享意图的用量通过 `physical_request_accounting` 去重统计。失败组耗时取当前调用墙钟时间与所有已记录请求时间总和的较大者，并标注 `elapsed_basis`；中断后恢复的耗时不冒充连续运行的端到端墙钟时间。基础设施失败较多时，端到端指标反映该次部署表现，需结合失败分类解读，不能直接归因为推荐能力差异。

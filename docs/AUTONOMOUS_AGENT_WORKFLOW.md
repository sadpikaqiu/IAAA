...
fix04 的模型原始响应使用 `ranked_pois_by_id` 对象：键为 10 个不同的 POI ID，每个值含模型给出的整数 `rank`、理由、affordance 和引用。动态 Schema 为各 POI 单独限定可引用的证据；原始 JSON 解析拒绝重复键，运行时要求名次恰好为 1–10 的排列。对象按 ID 顺序序列化，最终排名只按模型给出的 `rank` 解码；程序不去重补齐、不推断名次。保存给评估器的结果仍是按名次排列的 `ranked_pois` 数组。此输出协议变化可能影响模型行为，必须独立验证。
